...
export default function Navbar() {
  ...
  return (
    <>
      ...
      >
        ...
        <div 
          ref={glowRef}
          className="pointer-events-none absolute w-[300px] h-[300px] bg-cyan-400/10 rounded-full blur-3xl -translate-x-1/2 -translate-y-1/2 mix-blend-screen opacity-0 md:opacity-100"
        />
  ...
  <div className="relative w-10 h-10 flex items-center justify-center">
    <div className="absolute inset-0 rounded-full border border-red-500/40 group-hover:scale-110 transition-transform duration-500" />
    <div className="absolute inset-2 rounded-full border border-green-400/60" />
    <div className="w-2 h-2 rounded-full bg-cyan-400 shadow-[0_0_20px_rgba(34,211,238,0.8)]" />
  </div>

  <div className="flex flex-col leading-none">
    <span className="text-white font-semibold tracking-[0.25em] uppercase text-xs">
      Breaking  
    </span>
    <span className="text-green-400 font-bold text-[12px] tracking-[0.4em] uppercase">
      Bad  
    </span>
  </div>
</div>
          ...
          <div className="hidden md:flex items-center space-x-10">
            {NAV_LINKS.map((link, index) => (
              <a 
               ...
               className="relative text-gray-400 hover:text-cyan-400 text-sm font-medium tracking-[0.2em] uppercase transition-colors duration-300 group"
              >
                {link.name}
